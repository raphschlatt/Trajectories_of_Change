...
- `quickstart_colab.ipynb` loads the two-Parquet handoff, computes the
  multimetric overview, and displays the central Plotly figures.
...
`multiple_testing_scope="slice"`), passing some of them explicitly for
illustration. One deliberate deviation: it sets
`skip_incomplete_slices=False` to keep a trailing shortened time window.
